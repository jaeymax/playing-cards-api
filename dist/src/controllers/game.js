...
const createGame = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const { userId, winPoints, numPlayers, includeSixes, includeAces, isStakeGame, stake, } = req.body;
    // --------------------------------------------------
    // 1. VALIDATION
    // --------------------------------------------------
    ...
    `;
      ...
      SELECT
        ...
        ${(_a = row.player) === null || _a === void 0 ? void 0 : _a.id},
        -1,
        'in_deck'
      RETURNING
        id,
    ...
    `;
        ...
        // 8. BUILD GAME OBJECT
        // --------------------------------------------------
        const game = Object.assign(Object.assign({}, row), { players: row.player ? [row.player] : [], cards: gameCards, challenge: row.challenge || null, isStakeGame: Boolean(isStakeGame) });
        // Remove helper fields if you don't want them
        delete game.player;
        // --------------------------------------------------
        // 9. MIXPANEL
        // --------------------------------------------------
        __1.mixpanel.track("Game Created", Object.assign({ distinct_id: userId, game_code: gameCode, num_players: numPlayers, win_points: winPoints, game_type: isStakeGame ? "cash challenge" : "invite friend" }, (isStakeGame && {
            stake: numericStake,
...
const getUserGames = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let statusCondition = (0, db_1.default) ``;
        if (statusFilter === "waiting") {
            statusCondition = (0, db_1.default) `
      ...
      `;
         ...
         * -----------------------------------------
         */
        let typeCondition = (0, db_1.default) ``;
        /*
         * Cash challenge
         */
        if (typeFilter === "cash") {
            typeCondition = (0, db_1.default) `
      ...
      `;
         ...
         * -----------------------------------------
         */
        const countResult = yield (0, db_1.default) `
  ...
  ${typeCondition};
`;
        const total = Number((_c = (_b = countResult[0]) === null || _b === void 0 ? void 0 : _b.total) !== null && _c !== void 0 ? _c : 0);
    ...
    `;
        ...
        const formattedGames = games.map((game) => {
            ...
            const players = Array.isArray(game.players)
                ? game.players.map((player) => {
                    ...
                    return ({
                        id: Number(player.id),
                        username: player.username,
                        score: Number((_a = player.score) !== null && _a !== void 0 ? _a : 0),
                        position: Number(player.position),
                        is_dealer: Boolean(player.is_dealer),
                        player_status: player.player_status,
                        is_you: Boolean(player.is_you),
            ...
            return {
                 ...
                 * Challenge
                 */
                challenge_id: game.challenge_id !== null ? Number(game.challenge_id) : null,
                challenge_status: game.challenge_status || null,
                stake: game.stake !== null ? Number(game.stake) : null,
                platform_fee: game.platform_fee !== null ? Number(game.platform_fee) : null,
                ...
                current_turn_user_id: game.current_turn_user_id !== null
                    ? Number(game.current_turn_user_id)
                    : null,
                turn_started_at: game.turn_started_at || null,
                turn_timeout_seconds: game.turn_timeout_seconds !== null
                    ? Number(game.turn_timeout_seconds)
                    : null,
                forfeit_at: game.forfeit_at || null,
                forfeited_by: game.forfeited_by !== null ? Number(game.forfeited_by) : null,
            };
        });
        /*
         * -----------------------------------------
